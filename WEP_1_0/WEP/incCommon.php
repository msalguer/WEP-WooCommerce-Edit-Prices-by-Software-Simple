<?php
	...
	*/
	...
	function getTableList(){
		$arrTables=array(   
			'ss_view_product_description'=>array('WooCommerce Edit Prices', '', 'table.gif'),
			'ss_view_product_featured'=>array('Product Featured', '', 'table.gif'),
			'ss_view_product_price'=>array('Product Price', '', 'table.gif'),
			'ss_view_product_regular_price'=>array('Product regular price', '', 'table.gif'),
			'ss_view_product_sale_price'=>array('Product sale price', '', 'table.gif'),
			'ss_view_product_sale_price_dates_from'=>array('Product sale price dates from', '', 'table.gif'),
			'ss_view_product_sale_price_dates_to'=>array('Product sale price dates to', '', 'table.gif'),
			'ss_view_product_sku'=>array('Product Sku', '', 'table.gif')
			);
	...
	function logInMember(){
		...
		if($_POST['signIn']!=''){
			if($_POST['username']!='' && $_POST['password']!=''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('protecs6_1_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('protecs6_1_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['protecs6_1_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['protecs6_1_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="index.php"><i class="glyphicon glyphicon-home"></i> protecs6 1</a>
			</div>
