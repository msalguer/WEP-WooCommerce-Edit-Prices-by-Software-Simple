	...
	*/
	...
	if(!function_exists('getTableList')){
		function getTableList(){
			$arrTables=array(   
				"ss_view_product_description"=>"WooCommerce Edit Prices",
				"ss_view_product_featured"=>"Product Featured",
				"ss_view_product_price"=>"Product Price",
				"ss_view_product_regular_price"=>"Product regular price",
				"ss_view_product_sale_price"=>"Product sale price",
				"ss_view_product_sale_price_dates_from"=>"Product sale price dates from",
				"ss_view_product_sale_price_dates_to"=>"Product sale price dates to",
				"ss_view_product_sku"=>"Product Sku"
				);
	...
	function makeSafe($string, $is_gpc = true){
		...
		return db_escape($string, db_link());
	}
	########################################################################
	...
	function logOutUser(){
		...
		if(isset($_COOKIE['protecs6_1_rememberMe'])){
			setcookie('protecs6_1_rememberMe', '', time()-42000);
		}
		session_destroy();
	}
	########################################################################
	...
	function setupMembership(){
		...
		if(!$anonGroupID){
			...
			$anonGroupID = db_insert_id(db_link());
		}
		if($anonGroupID){
		...
		if(!$adminGroupID){
			...
			$adminGroupID = db_insert_id(db_link());
		}

		if($adminGroupID){
			// check that admins can access all tables
			$all_tables = array(  'ss_view_product_description' => 0, 'ss_view_product_featured' => 0, 'ss_view_product_price' => 0, 'ss_view_product_regular_price' => 0, 'ss_view_product_sale_price' => 0, 'ss_view_product_sale_price_dates_from' => 0, 'ss_view_product_sale_price_dates_to' => 0, 'ss_view_product_sku' => 0);
